...
- [x] 检测程序停止
- [x] 自动切换 active
- [x] duration 累计时长自动完成（核心）
- [x] 程序退出后保留累计时间
- [x] 再次启动后继续累计
- [x] process_start 自动完成（可选）
- [ ] 记录累计运行时间
...
软件基本可以使用，现在是优化细节。请给出codex以下任务的提示词

1.没有任务时，仅显示颜文字本体，取消颜文字旁边的白色圆长框，将框设置为display：none，或者其他方式，要求代码简洁

2.将设置页面从任务面板中剥离出来，形成下面所示的页面，其中（1）桌宠大小正常100%为 width:  192，height:  208,小则width：96px，height：104px，大则为width：240px，height：260px。（2）常规的开机自动启动调整，变成不用window登录也可以自动启动的，因为有些人会禁用window更新或者不登录导致这个开机启动失效，大多数软件都没有登录要求的。（3）数据目前应在该软件安装的盘下，如我安装软件在”d：\任务完成桌宠 “文件夹里，则数据目录自动放在这个文件夹里，不要放在c盘。

设置里桌宠模块，添加鼠标按键绑定功能：如鼠标左键点击pet时默认为打开任务面板，鼠标双击可设置为暂停任务，右键可以选择打开设置或者快速添加任务或者退出等等

![](./media/DESIGN/ChatGPT Image 2026年8月24日 16_05_41.png)



3. 在任务面板里添加任务计时功能，如任务-历史-总计时，总计时下有两个标题”今日计时“和”周计时“，选择“今日计时”则显示今天任务累计时的时间，并且任务按从累计时从大到小排序,周计时同理，为什么要累计时而不是目标计时，这是为了让用户知道自己学了多久，更有成就感，目标计时不明确，容易受外部影响，我们不能只完成目标，要有追求
4. 托盘快速添加任务选项，我觉得当前其原理为先打开任务面板--点击添加，如果任务面板存在则跳转到添加页面，但不存在时就跳转至任务面板页面，请调整为点击快速添加任务时，跳转至添加页面，用户立马可以设置任务，而不是在任务面板中再次点击添加按钮进行设置任务
5. 任务面板的今日任务里，如果绑定的程序的任务右边添加”启动“小按钮，将">"去掉，改为“启动按钮”，功能是用户点击"启动",执行任务选择的**.exe程序
6. 在添加任务页面里，请优化 （1）类型和完成方式的下拉框，让用户选择选择更美观，现代化。（2）在完成方式下添加另一个选项“启动即完成”。（3）绑定程序中的“选择正在运行”，在下面“当前程序”的旁边添加搜索功能，方便搜索列表下的exe文件，太多文件让用户选择困难
